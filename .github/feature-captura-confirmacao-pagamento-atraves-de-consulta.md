# Feature: Implementar Captura de Pagamento através de consulta

## Objetivo

Ao realizar uma consulta executando o caso de uso "ConsultarStatusPagamento" caso o status do pagamento tenha o status de pagamento confirmado de acordo com o payload recebido da Stripe deve mudar o status do pagamento para pago e enviar a consfirmação para a fila de pagamento confirmado

### Contexto

- Ao realizar uma consulta de status pagamento e o estado do pagamento esteja como confirmado (ou de acordo com a condição que indica pagamento recebido pela Stripe)
- Deve ser criado um caso de uso separado para manipular a mudança de estados do pagamento para confirmadou ou pago
- Deve ser alterado o status do pagamento no DynamoDb para pago
- Deve ser enviado o pagamento confirmado para uma fila AWS SQS para notificar os interessados no recebimento do pagamento
...
1. **Organizar Estrutura da Aplicação**
   - Criar estrutura de pastas para novo caso de uso de "ConfirmarPagamento", seguinte a estrutura dos demais.
   ...
   - Criar um serviço para encapsular a lógica de confirmação de pagamento.

2. **Alterar o status do pagamento no caso de uso ConfirmarPagamento**
   - Alterar o status do pagamento na tabela do DynamoDb "Pagamentos" para "Pago".
   - Caso falhe a alteração deverá haver uma notificaão de algum forma que ocorreu um erro e efetuado logs.
   - Utilizar o serviço existente IPagamentoRepository, caso não exista a implementação necessária criar

3. **Enviar mensagem de confirmação de pagamento no caso de uso ConfirmarPagamento**
   - Publicar mensagem no AWS SQS na fila "torne-se-fila-pagamento-confirmado"
